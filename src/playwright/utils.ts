    ...
    switch (screen[0].method) {
      case "POST":
        try {
          ...
          if (response.data.message === 'Status updated' && response.status === 200) {
...
export async function handleExternalService(apidata: any) {
  ...
  if (service.type === "polling") {
    ...
    while (Date.now() - startTime < timeoutMs) {
      ...
      try {
        ...
        if (success) {
          ...
          console.log(
            `ℹ️ ${i} Polling: status/body mismatch, retrying in ${intervalMs / 1000}s`
          );
        }
      } catch (error: any) {
    ...
    try {
      ...
      console.error(`❌ Stateless call error for ${service.name}:`, error.message);
      throw error;
    }
  } else {
